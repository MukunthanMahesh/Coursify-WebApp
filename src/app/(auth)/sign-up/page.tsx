...
export default function SignUp() {
  ...
  const resetAccount = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'Failed to reset account');
      setAccountConflict(false);
      toast({ title: "Account reset", description: "You can now try to sign up again", variant: "success" });
    } catch (error: any) {
      toast({ title: "Error resetting account", description: error.message || "Failed to reset account. Please try again.", variant: "destructive" });
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isQueensEmail(email)) {
      toast({ title: "Invalid email domain", description: "Please use your Queen's University email address (@queensu.ca)", variant: "destructive" });
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      toast({ title: "Verification email sent", description: "Please check your Queen's email to verify your account", variant: "success" });
    } catch (error: any) {
